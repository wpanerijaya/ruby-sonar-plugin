...
public class RubocopJsonParserImpl implements RubocopJsonParser {
	public List<RubocopOffense> parse(File file) throws IOException {
		...
		String fileString = FileUtils.readFileToString(rubocopResultFile,
				"UTF-8");
		...
		for (Object obj1 : fileArray) {
			...
			for (Object obj2 : offenseArray) {
				...
				String severity = offenseJsonObj.get("severity") != null ? offenseJsonObj
						.get("severity").toString() : null;
				String message = offenseJsonObj.get("message") != null ? offenseJsonObj
						.get("message").toString() : null;
				String copname = offenseJsonObj.get("cop_name") != null ? offenseJsonObj
						.get("cop_name").toString() : null;
				String corrected = offenseJsonObj.get("corrected") != null ? offenseJsonObj
						.get("corrected").toString() : null;
				JSONObject locationJsonObj = (JSONObject) offenseJsonObj
						.get("location");
				int line = locationJsonObj.get("line") != null ? Integer
						.parseInt(locationJsonObj.get("line").toString()) : 0;
				int column = locationJsonObj.get("column") != null ? Integer
						.parseInt(locationJsonObj.get("column").toString()) : 0;
				int length = locationJsonObj.get("length") != null ? Integer
						.parseInt(locationJsonObj.get("length").toString()) : 0;
